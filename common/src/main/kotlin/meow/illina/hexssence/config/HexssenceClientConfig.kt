package meow.illina.hexssence.config
...
object HexssenceClientConfig {
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            HexssenceServerConfig.holder.config = HexssenceServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            HexssenceServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = Hexssence.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use HexssenceServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = HexssenceServerConfig.ServerConfig()
    }
